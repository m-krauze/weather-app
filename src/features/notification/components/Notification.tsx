...
export function Notification(props: NotificationProps) {
  ...
  return (
    <div
      role="alert"
      className={`flex max-w-xs fixed top-1 right-1 alert alert${type ? `-${type}` : ""} opacity-1 transition-opacity duration-[2000ms] ease-in ${didMount ? 'opacity-0' : ""}`}
    >
      <div dangerouslySetInnerHTML={{ __html: getAlertIcon(type) }} />
      <span>{text}</span>
    </div>
  );
}
...
//  Utils
function getAlertIcon(type: NotificationProps["type"]) {
  switch (type){
  case "info":
